...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedConfigRaw) {
        ...
        if (parsed.success) {
          setLlmConfig(parsed.data);
        } else {
          console.error("Invalid LLM config in localStorage", parsed.error);
        }
      }
    } catch (e) {
        console.error("Failed to parse llmConfig from localStorage", e);
    }
  }, []);
  ...
  const handleSendMessage = async (messageContent: string) => {
    ...
    try {
      // For the API call, we need the history *before* the new user message was added.
